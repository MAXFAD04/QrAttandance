...
В скрипте `src/utils/seedDatabase.js` создаются тестовые пользователи (в т.ч. `admin@university.edu` / `admin123` и др.)

## Запуск с телефона в той же Wi‑Fi сети

1. Узнайте **локальный IP** ПК в сети Wi‑Fi (например `XXX.XXX.X.XX`). В Windows: `ipconfig`, раздел адаптера Wi‑Fi → IPv4.
